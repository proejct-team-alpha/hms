// Mock Data for Staff HTML pages
...
function getReservations() {
  const stored = localStorage.getItem('staff_reservations');
  if (stored) return JSON.parse(stored);
  localStorage.setItem('staff_reservations', JSON.stringify(mockReservations));
...
function updateReservationStatus(id, newStatus) {
  ...
  localStorage.setItem('staff_reservations', JSON.stringify(updated));
  return updated;
}

function addReservation(newData) {
  const reservations = getReservations();
  const newId = reservations.length > 0 ? Math.max(...reservations.map(r => r.id)) + 1 : 1;
  const newReservation = { id: newId, ...newData };
