...
- [x] 검색/필터 바 UI 추가
- [x] 활성 상태 배지 표시 추가
- [x] 결과 없을 때 빈 목록 메시지 추가
- [x] 필터/검색 조건 유지 페이지네이션 UI 반영

## Task 25-5. 테스트 보강
- [x] 컨트롤러 테스트에 파라미터 전달과 렌더링 검증 추가
- [x] 서비스 테스트에 필터/검색/정렬/빈 결과 검증 추가
...
- 검색 바는 제목 검색, 카테고리, 활성 여부, 조회 버튼, 초기화 버튼 구성으로 맞췄다.
- Mustache의 `selected` 처리를 위해 `AdminRuleListResponse`에 카테고리/활성 옵션 목록을 추가했다.
- 목록 row는 카드형 레이아웃을 유지하되 `categoryText`, `activeText`, `activeBadgeClass`를 함께 노출한다.
- 빈 결과일 때는 `조회된 규칙이 없습니다.` 메시지를 표시한다.
- 페이지네이션 링크는 `model.previousUrl`, `model.pageLinks`, `model.nextUrl`를 사용해 현재 필터 조건을 유지한다.

## Task 25-5 구현 메모
- `AdminRuleControllerTest`에 검색/필터 바 렌더링, 빈 결과 메시지, 선택된 필터 렌더링, 페이지 번호 렌더링 검증을 추가했다.
