...
- [ ] `RECEIVED` 상태 라벨을 사용자 노출 기준 `접수`로 정리한다.
- [ ] 상태 옵션 목록에서 `접수`가 자연스럽게 보이도록 정리한다.
- [ ] 상태 배지 텍스트도 동일한 용어 기준으로 맞춘다.
- [ ] 내부 상태값은 기존 `RECEIVED`를 유지하고, UI 라벨만 변경되도록 정리한다.
- [ ] 서비스 테스트에서 상태 라벨 기대값을 갱신한다.
...
- [ ] Task 21-2부터는 서비스 라벨 기대값과 템플릿 문구를 함께 정리하면 구현이 가장 자연스럽다.