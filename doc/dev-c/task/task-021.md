...
- [ ] `reservation-list.mustache`의 화면 설명 문구를 예약/접수 통합 흐름에 맞게 정리한다.
- [ ] 상태 필터 UI에서 `접수` 라벨이 자연스럽게 보이도록 정리한다.
- [ ] 빈 목록 메시지와 상태 배지 문구가 새 용어와 충돌하지 않도록 점검한다.
- [ ] 페이지네이션과 성공/실패 메시지 영역은 기존 패턴을 유지한다.
- [ ] 인코딩 손상 없이 템플릿을 UTF-8 기준으로 저장한다.

## Task 21-4. 취소 후 복귀 흐름과 필터 유지 검증
- [ ] 예약 취소 후 현재 `page`, `size`, `status`가 유지되는지 확인한다.
- [ ] `접수` 필터 상태에서 취소 후에도 같은 필터로 복귀하는지 확인한다.
- [ ] 잘못된 상태 파라미터가 들어와도 기본 동작이 안전한지 점검한다.
- [ ] 필요 시 컨트롤러 리다이렉트 파라미터 처리를 정리한다.
- [ ] 관련 컨트롤러 테스트를 보강한다.
