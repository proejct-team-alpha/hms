...
## 목적
- [ ] `workflow-023`의 환자 정보 부분 수정 REST API 범위를 실제 구현 가능한 작업 단위로 분해한다.
- [ ] 관리자 환자 수정 API를 `POST /admin/api/patients/{id}/update` 기준으로 정리한다.
- [ ] 수정 가능 필드를 `name`, `phone`, `note`로 제한한다.
- [ ] 기본 검증, 전화번호 중복 방지, 공통 API 응답 패턴을 함께 정리한다.
- [ ] `admin.patient` 범위 API 테스트와 문서를 순서대로 마무리한다.

## Task 23-1. 현재 환자 수정 가능 구조 점검
- [ ] `Patient` 엔티티의 수정 가능 필드와 현재 변경 메서드를 점검한다.
- [ ] 전화번호 중복 판단 기준과 현재 환자 재사용 규칙을 확인한다.
- [ ] `admin.patient` 범위에 API 컨트롤러가 필요한지 구조를 정리한다.
- [ ] 현재 `Resp`, `CustomException`, `GlobalExceptionHandler` 패턴을 다시 확인한다.
- [ ] 없는 `patientId`, 공백 입력, 중복 전화번호 시나리오를 메모한다.
...
- [ ] 기존 전화번호와 동일한 값으로 저장하는 경우는 정상 허용한다.
...
## 메모
- [ ] 수정 가능 필드는 `name`, `phone`, `note`로 제한한다.
- [ ] 현재 환자 동일성 판단은 사실상 전화번호 기준이므로, 수정 시 중복 전화번호 방지가 핵심이다.
- [ ] `note`는 선택 입력이므로 null/blank 허용 정책을 구현 전에 정리한다.
- [ ] API는 `/admin/api/**` 범위에 두고 SSR 기능과 책임을 분리한다.
- [ ] 응답은 기존 프로젝트 공통 포맷인 `Resp.ok(...)`를 유지한다.
- [ ] 향후 이메일/주소 수정, 패턴 검증, SSR 수정 폼은 별도 확장 과제로 둔다.
