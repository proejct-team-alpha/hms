...
- [x] 전화번호 중복 판단 기준과 현재 환자 동일성 규칙을 확인한다.
...
- [x] 응답 DTO는 수정 후 필요한 환자 정보만 담도록 범위를 정리한다.

## Task 23-3. 환자 수정 서비스 로직 구현
- [x] `AdminPatientService`에 환자 수정 메서드를 추가한다.
- [x] 없는 `patientId`는 not found 예외로 처리한다.
- [x] `name`, `phone`, `note` 수정 로직을 구현한다.
- [x] 자기 자신을 제외한 다른 환자와 전화번호가 중복되면 수정을 차단한다.
...
- [x] 성공 수정 시나리오를 검증한다.
- [x] `name` 공백 입력 검증 실패를 검증한다.
- [x] `phone` 공백 입력 검증 실패를 검증한다.
- [x] 전화번호 중복 수정 차단 시나리오를 검증한다.
- [x] 없는 `patientId` 수정 요청 시나리오를 검증한다.
...
- [x] 관련 `admin.patient` API 테스트가 통과한다.

## 추천 진행 순서
- [x] Task 23-1 현재 환자 수정 관련 구조 점검
...
- [x] 현재 환자 동일성 판단과 재사용 규칙은 전화번호 기반이다. 온라인 예약, 전화 예약, 방문 접수 모두 전화번호로 기존 환자를 찾고 없으면 `Patient`를 생성한다.
- [x] `AdminPatientRepository`에는 현재 목록 검색과 상세 이력 조회만 있고, 전화번호 중복 확인용 메서드는 별도로 추가해야 했다.
- [x] `admin.patient` 범위에는 기존 API 컨트롤러가 없었으므로 프로젝트 패턴에 맞는 `AdminPatientApiController`가 새로 필요했다.
- [x] 관리자 API 패턴은 `@RequestMapping("/admin/api/...")`, `@Valid @RequestBody`, `Resp.ok(...)` 응답 형식으로 유지한다.
- [x] `GlobalExceptionHandler`는 `@RestController` 범위에서 `MethodArgumentNotValidException`, `CustomException` 등을 `Resp.fail(...)` 형식으로 처리한다.
- [x] 주요 예외 시나리오는 없는 `patientId`, `name`/`phone` 공백 입력, 다른 환자와 전화번호 중복이다.
- [x] `note`는 선택 입력으로 두고, DTO 단계에서는 `null/blank` 모두 허용했다. 실제 저장 시 정규화 정책은 서비스 구현 단계에서 정리했다.
- [x] `UpdateAdminPatientApiRequest`는 `name`, `phone`, `note`만 받고, 응답 DTO는 `patientId`, `name`, `phone`, `note`, `message`만 담는 작은 구조로 정리했다.
- [x] `AdminPatientService.updatePatient(...)`는 `name`, `phone`, `note`만 수정하고 기존 `email`, `address`는 보존하도록 구현했다.
- [x] 전화번호 비교는 하이픈과 공백 제거 기준으로 자기 자신 제외 중복을 검사하도록 `AdminPatientRepository.existsByNormalizedPhoneAndIdNot(...)`를 추가했다.
