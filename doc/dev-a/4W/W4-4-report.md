...
> **💡 입문자 설명**
> - **이 코드가 하는 일**: 여러 Entity와 Repository를 추가한 후 JPA가 DB 테이블 매핑을 스캔할 때 오류가 없음을 확인합니다.
> - **왜 이렇게 썼는지**: JPA Entity를 여러 개 추가하면 테이블 이름 중복이나 관계 설정 오류가 빌드/서버 기동 시 발생할 수 있습니다. 빌드 성공은 이런 문제가 없음을 의미합니다.
> - **쉽게 말하면**: 새 가구를 여러 개 들여놓은 후 집에 아무 문제가 없는지 확인하는 것입니다.
