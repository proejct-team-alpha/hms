...
> **💡 입문자 설명**
> - **이 코드가 하는 일**: 물품 목록을 10개씩 페이지로 나눠 표시합니다. 검색창에 글자를 입력하면 초성·영문 매칭으로 물품을 필터링하고, 필터링된 결과를 다시 1페이지부터 보여줍니다.
> - **왜 이렇게 썼는지**: `PAGE_SIZE`를 상수로 정의하면 나중에 10을 20으로 바꿀 때 한 곳만 수정하면 됩니다. `slice(start, start + PAGE_SIZE)`는 전체 배열에서 현재 페이지에 해당하는 부분만 잘라냅니다. `addEventListener('input', ...)`는 글자를 입력할 때마다 실시간으로 검색이 동작합니다.
> - **쉽게 말하면**: 전체 물품 목록을 10개짜리 쪽으로 잘라서 보여주고, 검색어를 입력하면 해당 물품만 걸러서 다시 1쪽부터 보여주는 기능입니다.
...
> **💡 입문자 설명**
> - **이 코드가 하는 일**: 입출고 내역 테이블을 10개씩 페이지로 나눠 표시하는 기본 구조입니다. `renderPage(page)`가 호출되면 해당 페이지의 행만 보여주고 나머지는 숨기며, 번호 버튼도 함께 갱신합니다.
> - **왜 이렇게 썼는지**: 기존에 15개씩 이전/다음 버튼만 있던 것을 10개씩 + 번호 버튼 방식으로 교체했습니다. `start = (page - 1) * PAGE_SIZE`는 페이지 번호로 배열 시작 인덱스를 계산하는 공식입니다(1페이지: 0, 2페이지: 10, 3페이지: 20...).
> - **쉽게 말하면**: 전체 내역을 10줄씩 묶어 책처럼 페이지를 넘기는 기능입니다.
