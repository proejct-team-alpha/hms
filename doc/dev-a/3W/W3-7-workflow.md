...
1. C-05: direct-reservation fetch async/await 전환 + response.ok 검사
2. H-01: IDOR 방어 — 예약 취소/변경 시 phone 소유권 검증
3. H-02: 예약번호 생성 ReservationNumberGenerator 통일
4. H-03: TOCTOU 방어 — 비관적 락(@Lock PESSIMISTIC_WRITE) 적용
5. H-06: Admin 전용 쿼리 ReservationRepository에서 제거
6. H-13: DTO 이름 변경 (ReservationCreateForm → CreateReservationRequest 등)
7. M-03: @Pattern 검증 추가 (phone, timeSlot)
8. M-10: index.mustache 인라인 header/footer → 파셜 교체
9. M-11: 진료과 드롭다운 서버 바인딩
...
6. ReservationServiceTest — getBookedTimeSlots 테스트 2건 추가
