...
> **문서 버전:** v3.0
> **작성일:** 2026년
> **변경 이력:** v2.1 (3개 분권, 61개) → **v3.0 (단일 문서, 컨트롤러 반환 패턴 전면 개정)**
...
v3.0부터 모든 컨트롤러는 아래 패턴을 따릅니다. JSON 응답 바디(`{success, data, message}`)는 **LLM 엔드포인트와 비동기 조회 API(getSlots·getDoctors)에만** 유지합니다.
...
| 항목 | 내용                                                                              |
| ---- | --------------------------------------------------------------------------------- |
| 설명 | 비활성 진료과를 활성화 (is_active = TRUE) — 예약 화면 및 LLM 프롬프트에 즉시 반영 |
| 인증 | ROLE_ADMIN                                                                        |
...
| #   | 메서드 | URL                          | 반환                        | 설명                |
| --- | ------ | ---------------------------- | --------------------------- | ------------------- |
| 21  | GET    | `/doctor/dashboard`          | `"doctor/dashboard"`        | DOCTOR 대시보드     |
| 22  | GET    | `/doctor/treatment/list`     | `"doctor/treatment/list"`   | 오늘 진료 목록 화면 |
| 23  | GET    | `/doctor/treatment/detail`   | `"doctor/treatment/detail"` | 진료 기록 입력 화면 |
| 24  | POST   | `/doctor/treatment/complete` | redirect or 폼재렌더링      | 진료 완료 처리      |

### ROLE_NURSE (간호사)

| #   | 메서드 | URL                     | 반환                     | 설명                     |
| --- | ------ | ----------------------- | ------------------------ | ------------------------ |
| 25  | GET    | `/nurse/dashboard`      | `"nurse/dashboard"`      | NURSE 대시보드           |
| 26  | GET    | `/nurse/schedule/list`  | `"nurse/schedule/list"`  | 오늘 예약 현황 화면      |
| 27  | GET    | `/nurse/patient/detail` | `"nurse/patient/detail"` | 환자 정보 조회·수정 화면 |
| 28  | POST   | `/nurse/patient/update` | redirect or 폼재렌더링   | 환자 정보 수정 처리      |

### ROLE_DOCTOR + ROLE_NURSE (LLM 챗봇)

| #   | 메서드 | URL                  | 반환        | 설명                     |
| --- | ------ | -------------------- | ----------- | ------------------------ |
| 29  | POST   | `/llm/rules/ask`     | JSON (AJAX) | 규칙 Q&A 챗봇 질의       |
| 30  | GET    | `/llm/rules/history` | JSON (AJAX) | 현재 세션 챗봇 이력 조회 |

### ROLE_ADMIN — 대시보드·예약·환자

| #   | 메서드 | URL                         | 반환                       | 설명                 |
| --- | ------ | --------------------------- | -------------------------- | -------------------- |
| 31  | GET    | `/admin/dashboard`          | `"admin/dashboard"`        | 관리자 대시보드 화면 |
| 32  | GET    | `/admin/dashboard/stats`    | JSON (AJAX)                | 대시보드 통계 데이터 |
| 33  | GET    | `/admin/reception/list`     | `"admin/reception/list"`   | 전체 접수 목록 화면  |
| 34  | GET    | `/admin/reservation/list`   | `"admin/reservation/list"` | 전체 예약 목록 화면  |
| 35  | POST   | `/admin/reservation/cancel` | redirect or Flash          | 예약 취소 처리       |
| 36  | GET    | `/admin/patient/list`       | `"admin/patient/list"`     | 환자 목록 화면       |
| 37  | GET    | `/admin/patient/detail`     | `"admin/patient/detail"`   | 환자 상세·이력 화면  |

### ROLE_ADMIN — 직원 관리

| #   | 메서드 | URL                       | 반환                   | 설명                |
| --- | ------ | ------------------------- | ---------------------- | ------------------- |
| 38  | GET    | `/admin/staff/list`       | `"admin/staff/list"`   | 직원 목록 화면      |
| 39  | GET    | `/admin/staff/new`        | `"admin/staff/new"`    | 직원 등록 화면      |
| 40  | POST   | `/admin/staff/create`     | redirect or 폼재렌더링 | 직원 등록 처리      |
| 41  | GET    | `/admin/staff/detail`     | `"admin/staff/detail"` | 직원 상세·수정 화면 |
| 42  | POST   | `/admin/staff/update`     | redirect or 폼재렌더링 | 직원 정보 수정 처리 |
| 43  | POST   | `/admin/staff/deactivate` | redirect               | 직원 비활성화 처리  |

### ROLE_ADMIN — 진료과 관리

| #   | 메서드 | URL                            | 반환                        | 설명                  |
| --- | ------ | ------------------------------ | --------------------------- | --------------------- |
| 44  | GET    | `/admin/department/list`       | `"admin/department/list"`   | 진료과 목록 화면      |
| 45  | POST   | `/admin/department/create`     | redirect or 폼재렌더링      | 진료과 등록 처리      |
| 46  | GET    | `/admin/department/detail`     | `"admin/department/detail"` | 진료과 상세·수정 화면 |
| 47  | POST   | `/admin/department/update`     | redirect or 폼재렌더링      | 진료과 수정 처리      |
| 48  | POST   | `/admin/department/deactivate` | redirect                    | 진료과 비활성화 처리  |
| 49  | POST   | `/admin/department/activate`   | redirect                    | 진료과 활성화 처리    |

### ROLE_ADMIN — 물품 관리

| #   | 메서드 | URL                  | 반환                   | 설명                |
| --- | ------ | -------------------- | ---------------------- | ------------------- |
| 50  | GET    | `/admin/item/list`   | `"admin/item/list"`    | 물품 목록 화면      |
| 51  | GET    | `/admin/item/new`    | `"admin/item/new"`     | 물품 등록 화면      |
| 52  | POST   | `/admin/item/create` | redirect or 폼재렌더링 | 물품 등록 처리      |
| 53  | GET    | `/admin/item/detail` | `"admin/item/detail"`  | 물품 상세·수정 화면 |
| 54  | POST   | `/admin/item/update` | redirect or 폼재렌더링 | 물품 전체 수정 처리 |
| 55  | POST   | `/admin/item/delete` | redirect               | 물품 삭제 처리      |

### ROLE_ADMIN — 병원 규칙 관리

| #   | 메서드 | URL                        | 반환                   | 설명                     |
| --- | ------ | -------------------------- | ---------------------- | ------------------------ |
| 56  | GET    | `/admin/rule/list`         | `"admin/rule/list"`    | 병원 규칙 목록 화면      |
| 57  | GET    | `/admin/rule/new`          | `"admin/rule/new"`     | 병원 규칙 등록 화면      |
| 58  | POST   | `/admin/rule/create`       | redirect or 폼재렌더링 | 규칙 등록 처리           |
| 59  | GET    | `/admin/rule/detail`       | `"admin/rule/detail"`  | 병원 규칙 상세·수정 화면 |
| 60  | POST   | `/admin/rule/update`       | redirect or 폼재렌더링 | 규칙 수정 처리           |
| 61  | POST   | `/admin/rule/toggleActive` | redirect               | 규칙 활성화 토글         |
| 62  | POST   | `/admin/rule/delete`       | redirect               | 규칙 삭제 처리           |
...
_본 API 명세서는 프로젝트 계획서 v4.2, ERD v2.0, 화면 정의서 v2.0을 기반으로 작성되었습니다._
